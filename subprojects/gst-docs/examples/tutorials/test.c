...
//    g_signal_connect(appsink, "new-sample", G_CALLBACK(on_new_sample_from_sink), s);
 ...
 GstCaps *klvCaps = gst_caps_new_simple("meta/x-klv",
   ...
   //g_signal_connect(dataAppSrc, "need-data", G_CALLBACK(pushKlv), NULL);
    ...
    /* Build the pipeline */
    gst_bin_add_many(GST_BIN(pipeline), videotestsrc, dataAppSrc, videoconvert, x264enc, h264parse, /*rtph264pay,*/ capsfilter,/* klvcapsfilter, mpegtsmux, appsink,*/udpsink, fdsink, NULL);
