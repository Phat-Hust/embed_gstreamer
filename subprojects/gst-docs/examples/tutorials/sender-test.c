...
static gboolean push_metadata(gpointer user_data) {
    ...
    g_print("Frame %d: Muxed KLV metadata '%s' (size: %zu bytes) at PTS: %" GST_TIME_FORMAT "\n", 
            counter, meta_str, strlen(meta_str), 
...
int main(int argc, char *argv[]) {
    ...
    g_object_set(mux, "alignment", 7, NULL);
    
    udp = gst_element_factory_make("udpsink", NULL);
    g_object_set(udp, "host", "127.0.0.1", "port", 5000, NULL);
    ...
    g_timeout_add(1000 / 10, push_metadata, metasrc); // ~30fps
